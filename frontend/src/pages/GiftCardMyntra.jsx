...
const GiftCardMyntra = () => {
  ...
  useEffect(() => {
    const fetchMyntraProducts = async () => {
      try {
        setLoading(true)
        let items = []
        const searchRes = await fetch(`${BACKEND_URL}/api/products/search?q=Myntra&limit=200`)
        const searchData = await searchRes.json()
        if (searchData.success && searchData.data && searchData.data.length > 0) {
          items = searchData.data
        } else {
          const catRes = await fetch(`${BACKEND_URL}/api/products/category/gift-cards?brand=Myntra&limit=200`)
        ...
        const active = items.filter(p => p.isActive !== false && p.brand && p.brand.toLowerCase().includes('myntra'))
        ...
        const mapped = Array.from(denomMap.values()).map(p => {
          ...
          return {
            ...p,
            name: p.seoTitle || p.name || `Myntra ₹${denom} Gift Card`,
            brand: 'Myntra'
          }
        ...
        console.error('Error fetching Myntra products:', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  useSEO({
    ...
    description: 'Get Myntra gift card codes instantly. Save on Myntra vouchers with instant digital delivery via email on GCHub.',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 p-8 text-center max-w-md mx-auto shadow-sm">
            ...
            <p className="text-sm text-slate-500">No Myntra gift card variants are currently available. Please check back later!</p>
          </div>
        ) : (
              ...
              return (
                <div key={voucher._id} onClick={() => navigate('/gift-card/myntra/detail')} className="group relative bg-white rounded-2xl border border-slate-200/80 overflow-hidden hover:border-emerald-300 hover:shadow-2xl hover:shadow-emerald-200/50 transition-all duration-300 flex flex-col cursor-pointer">
                  ...
                  <div className="aspect-[4/3] sm:aspect-[1.6/1] w-full overflow-hidden bg-slate-100 flex items-center justify-center relative">
                    <img src={voucher.images?.[0] || '/products/myntra.avif'} alt={voucher.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
